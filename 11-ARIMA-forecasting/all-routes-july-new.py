...
OUT_DIR = r"new_new_forecast_outputs"
...
def fit_forecast_model(series: pd.Series, forecast_steps: int = 12) -> dict:
    ...
    """
    def _split_conf_int(conf_int_obj):
        conf_arr = np.asarray(conf_int_obj)
        if conf_arr.ndim != 2 or conf_arr.shape[1] < 2:
            raise ValueError("Invalid confidence interval shape")
        return conf_arr[:, 0], conf_arr[:, 1]
    ...
    try:
        ...
        lower_ci, upper_ci = _split_conf_int(conf_int)
        ...
        try:
            ...
            forecast = np.asarray(forecast_obj.predicted_mean).ravel()
