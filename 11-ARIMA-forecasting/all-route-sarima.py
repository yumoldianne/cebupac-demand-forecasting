...
def load_and_prepare_data(path: str) -> pd.DataFrame:
    ...
    # Rename your actual columns to easier names
    rename_map = {
        "DATE": "Date",
        "FROM": "From",
        "TO": "To",
    ...
    # IMPORTANT:
    # Your sample does not include an explicit passenger-count column.
    # This script uses Load_Factor as the forecast target because it is the only available demand-like numeric field.
    # If you actually have passenger counts somewhere else, replace this line with that column.
    df["Passengers"] = pd.to_numeric(df["Load_Factor"], errors="coerce")

    # Monthly timestamp for forecasting
    df["Month_Date"] = df["Date"].dt.to_period("M").dt.to_timestamp()

    # Drop unusable rows
    df = df.dropna(subset=["Date", "Route", "Passengers"])
...
def auto_arima_forecast(series: pd.Series, forecast_steps: int = 12) -> dict:
    """Fit auto ARIMA and return forecast as numpy arrays."""
    try:
        ...
        print(f"    ⚠ Auto ARIMA failed: {str(e)[:80]}")
        return {"success": False, "error": str(e)}


def manual_sarima_forecast(series: pd.Series, forecast_steps: int = 12) -> dict:
    """Fallback SARIMA model."""
    try:
        y = pd.Series(series).astype(float).dropna().values
        ...
        print(f"    ⚠ Manual SARIMA failed: {str(e)[:80]}")
        return {"success": False, "error": str(e)}
...
def plot_route_forecast(
    ...
    ax.set_title(f"{route} — Passenger Forecast (Total Market vs Cebu Pacific)",
...
def main():
    ...
    for idx, route in enumerate(sorted(cebu_routes), 1):
        ...
        total_result = auto_arima_forecast(total_pax, FORECAST_MONTHS)
        if not total_result["success"]:
            print("    Trying manual SARIMA...")
            total_result = manual_sarima_forecast(total_pax, FORECAST_MONTHS)
        if not total_result["success"]:
            print("  ✗ Total market forecast failed")
            continue

        print(f"    ✓ Model: SARIMA{total_result['order']}{total_result['seasonal_order']} (AIC: {total_result['aic']:.0f})")

        # Forecast Cebu Pacific
        print("  Forecasting Cebu Pacific...")
        cebu_result = auto_arima_forecast(cebu_pax, FORECAST_MONTHS)
        if not cebu_result["success"]:
            print("    Trying manual SARIMA...")
            cebu_result = manual_sarima_forecast(cebu_pax, FORECAST_MONTHS)
        if not cebu_result["success"]:
            print("  ✗ Cebu Pacific forecast failed")
            continue

        print(f"    ✓ Model: SARIMA{cebu_result['order']}{cebu_result['seasonal_order']} (AIC: {cebu_result['aic']:.0f})")

        # Forecast dates
        last_date = dates.iloc[-1]
        forecast_dates = [last_date + pd.DateOffset(months=i + 1) for i in range(FORECAST_MONTHS)]

        # Make sure everything is array-indexable
