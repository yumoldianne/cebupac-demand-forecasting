"""
VERSION 2: LSTM FOR ALL DESTINATION AIRPORTS (UNIVARIATE)
==========================================================
Same structure as Version 1, but aggregates by DESTINATION airport (To column)
instead of origin (From column).

This answers: "How many passengers will ARRIVE at each airport next month?"
versus V1 which answers: "How many passengers will DEPART from each airport?"

Runs all 10 LSTM variants on EVERY destination airport in the dataset.
Tests window sizes 3, 6, 12 months per airport.
Target: Passenger counts arriving at destination (univariate time series)

Run:
    python lstm_v2_destination_airports.py
...
OUT_DIR      = 'lstm_v2_destination_airports'
...
def load_and_aggregate(path: str) -> pd.DataFrame:
    """
    Load CSV and aggregate by DESTINATION airport (To column).
    Sums passengers arriving at each destination per month.
    """
    df = pd.read_csv(path)
    ...
    # KEY DIFFERENCE: Aggregate by 'To' (destination) instead of 'From' (origin)
    agg = (
        df.groupby(['To', 'Date'], sort=True)[TARGET]
          .sum()
          .reset_index()
          .rename(columns={'To': 'Airport'})
    )
    agg = agg.sort_values(['Airport', 'Date']).reset_index(drop=True)
    
    print(f"  Raw rows             : {len(df):,}")
    print(f"  Total destinations   : {df['To'].nunique()}")
    print(f"  Date range           : {df['Date'].min().date()} → {df['Date'].max().date()}")
    print(f"  Aggregated rows      : {len(agg):,}")
    print(f"  Aggregation basis    : DESTINATION airports (To column)")
    return agg
...
def main():
    W = 80
    print('\n' + '═'*W)
    print('  VERSION 2: LSTM FOR ALL DESTINATION AIRPORTS (UNIVARIATE)')
    print('  Predicting passenger ARRIVALS (aggregated by To column)')
    print('  10 variants × ALL destination airports × 3 window sizes')
    print('═'*W + '\n')

    # ── Load & aggregate by DESTINATION ───────────────────────────────────────
    print('► Step 1 — Load & aggregate by DESTINATION')
    ...
    print(f'\n► Step 2 — Filter destination airports (minimum {MIN_MONTHS} months)')
    print(f'  Total destinations     : {len(airport_counts)}')
    print(f'  Destinations with ≥{MIN_MONTHS}m: {len(valid_airports)}')
    print(f'  Excluded               : {len(airport_counts) - len(valid_airports)}')
    
    if len(valid_airports) == 0:
        print('\n⚠ No destinations meet minimum data requirement. Exiting.')
        return
    
    # Sort by total passenger volume arriving
    ...
    print(f'\n  Processing {len(valid_airports)} destination airports...')
    print(f'  (Sorted by total arriving passengers)')
    ...
    for a_idx, airport in enumerate(valid_airports, 1):
        if a_idx % 10 == 1 or a_idx == len(valid_airports):
            print(f'{"─"*W}')
            print(f'  Progress: [{a_idx}/{len(valid_airports)}]  Destination: {airport}')
    ...
    print(f'\n{"─"*W}')
    print(f'  Destinations processed : {airports_processed}')
    print(f'  Destinations skipped   : {airports_skipped}')
    ...
    # Best overall per destination airport
    best_per_airport = (
    ...
    print(f'\n  Best model per destination airport (top 20):')
    ...
    print(f'  ★ Avg R² (all destinations)   : {test_df["R2"].mean():.4f}')
    ...
    axes[0].set_title('Distribution of Test R² (All Destinations)', fontweight='bold')
    axes[0].set_xlabel('R²')
    ...
    axes[1].set_title('Distribution of Test R² (Best Window per Destination)', fontweight='bold')
    axes[1].set_xlabel('R²')
    ...
    # Plot 3: R² by destination airport (top 30 by passenger volume)
    ...
    ax.set_title('Test R² by Destination Airport (Top 30 by Arriving Passengers)', 
                 fontweight='bold')
    ax.axvline(0, color='black', linestyle='-', linewidth=0.8)
    ax.grid(axis='x', alpha=0.3)
    plt.tight_layout()
    fig.savefig(f'{OUT_DIR}/fig3_r2_by_destination_top30.png', dpi=200)
    plt.close(fig)
    print('  ✓ fig3_r2_by_destination_top30.png')
    ...
    print('\n' + '═'*W)
    print('  DONE - Results are for DESTINATION airports (arrivals)')
    print('═'*W + '\n')
